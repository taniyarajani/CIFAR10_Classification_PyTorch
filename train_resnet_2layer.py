...
# Importing required package to get resnet
...
parser.add_argument('--model', choices=['softmax', 'convnet', 'twolayernn', 'mymodel', 'resnet_2layer'], help='which model to train/evaluate')
...
model.fc = nn.Linear(num_ftrs, args.hidden_dim)
model.relu1 = nn.ReLU(inplace = True)

# One hidden layer to final classes connection
model.fc1 = nn.Linear(args.hidden_dim, n_classes)
model.soft1 = nn.Softmax(1)
